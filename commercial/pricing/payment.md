...
- **Delivery model: GitHub collaborator invite to `ylyl6561/poly-hedging-pro`
  (private repo).** No zip file needed — the Pro repo IS the deliverable.
- License keys: enabled → generate automatically (used as proof-of-purchase
  reference, not as a runtime secret in the Pro repo)
- **Post-purchase success URL:** `https://github.com/ylyl6561/poly-hedging#-get-the-toolkit`
  — buyer lands back on the public README, sees the Free vs Pro table and
  the "GitHub invite arrives within 24h" line.
- Tags: `polymarket`, `trading`, `toolkit`, `python`, `hedging`
- **Bonus (first 20 buyers):** Free 30-min 1-on-1 onboarding call (Zoom).
  Send the first 20 buyers a separate email with your Cal.com / Calendly
  link after you send the GitHub invite. After 20, stop sending — keeps
  it scarce and honest.
...
Within 24 hours, you'll receive a GitHub invitation to the private
poly-hedging-pro repo at the email you used for payment. Accept it and
clone with your GitHub account:

  git clone https://github.com/ylyl6561/poly-hedging-pro.git
  cd poly-hedging-pro && pip install -r requirements.txt

License key: [auto-generated by Creem]  (proof-of-purchase reference)
Discord invite: https://discord.gg/YOUR_DISCORD_LINK
Full docs: https://github.com/ylyl6561/poly-hedging-pro

Questions? Reply to this email or ping us in Discord.
```

> Note: don't promise the GitHub invite will be instantaneous — give
> yourself 24h buffer. Also do NOT mention the first-20 onboarding call
> bonus in this auto email (it would mislead buyers 21+). Send the bonus
> reply manually. See `commercial/admin/launch-sop.md` for the templates.
...
## What You Ship (no zip — GitHub invite is the deliverable)

There is no zip to ship. The Pro `poly-hedging-pro` private repo IS the
deliverable. The buyer's license key serves as a proof-of-purchase
reference, not a runtime secret.
...
1. Auto Creem email: license key + "GitHub invite in 24h"
2. (You send) GitHub collaborator invite to poly-hedging-pro (within 24h)
3. (You send, buyers 1-20 only) Onboarding call Cal.com link
```

The Pro repo already contains:
...
License-key verification (optional) — see the snippet below for
`tools/verify_license.py`. Keep the HMAC secret out of any public repo.

...
- **Issue license keys:** ✅ Enabled (Creem's built-in license-key
  generator — each key is HMAC-signed and maps back to the order in
  your dashboard).
...
> Note: Creem's built-in license-key email is sufficient — **do NOT
> promise a GitHub invite** in this auto email (the Pro tier is gated
> by the license key, not by repo access). Also do NOT mention the
> first-20 onboarding call bonus in this auto email (would mislead
...
valid key, Pro paths exit early — **but the dry-run paths, the
calculator UI, the replay tool, and the source-code reading experience
all keep working** for free users.

License-key verification — see `tools/verify_license.py`. Keep the
HMAC secret out of any public repo (it's loaded from
`POLY_LICENSE_SECRET` env var in prod, with a dev fallback for CI).
